...
fluidPage(
  ...
  # Evsi
  tags$style(type='text/css', "#likelihood\\.psa\\.new\\.sample\\.add { margin-top: 25px;}"),
  tags$style(type='text/css', "#likelihood\\.psa\\.new\\.sample\\.calculate\\.evsi { margin-top: 25px;}"),
  tags$style(type='text/css', '#likelihood\\.psa\\.new\\.sample\\.error\\.message {margin-top: 25px;color: red;}'),
...
#          mainPanel(
      ...
      tabPanel("EVSI", 
               h3("Simulate a new sample based on PSA parameter(s)"),
               sidebarLayout(position = "left",
                             sidebarPanel(
                               #conditionalPanel(condition = "output['has.no.parameters'] == true",
                               #                 "Please import your psa sample with at least on parameter first!"),
                               #Using modal
                               bs_modal("idModal",   htmlOutput("scenarioName"),  htmlOutput("scenarioDescriptionLatex"), footer = bs_modal_closebutton(title = "Close"),
                                        size = "medium"), 
                              # conditionalPanel(condition = "output['has.no.parameters'] == false",
                                                fluidRow(column(12, selectInput("likelihood.psa.new.sample","Likelihood for parameter:", list())%>%
                                                ...
                                                uiOutput("scenario"),
                                                fluidRow(column(4, numericInput("likelihood.psa.new.sample.size.start", 
                                                                                                 "Start: ", value = 3, 
                                                                                                 min = 3, step = 10)
                                                        ),
                                                        column(4, numericInput("likelihood.psa.new.sample.size.end", 
                                                                                        "End: ", value = 100, 
                                                                                        max = 10000, step = 10)
                                                         ),
                                                        column(4,numericInput("likelihood.psa.new.sample.size.step", 
                                                                                       "Steps: ", value = 10, 
                                                                                       max = 100, step = 1)
                                                 )), 
                                                 fluidRow(column(6, numericInput("likelihood.psa.new.sample.replication", 
                                                                                                 "Replication(s): ", value = 1, 
                                                                                                 min = 1, max= 3, step = 1)
                                                 )),
                                                 fluidRow(column(3, actionButton("likelihood.psa.new.sample.calculate.evsi", 
                                                                                                 "calculate EVSI")),
                                                           column(6, textOutput("likelihood.psa.new.sample.error.message")))
                               
                               , width=6),
                             mainPanel(
                               tabsetPanel(type="tab",
                                           tabPanel("Plot", plotlyOutput("plotEVSI")),
                                           tabPanel("Table", tableOutput("tableEVSI")))
                               , width=6)
               )
      ),
